...
// MySQL service form schema with storage capacity and instance type
const mysqlServiceFormSchema = z.object({
...
// Form types
type MysqlServiceFormValues = z.infer<typeof mysqlServiceFormSchema>;

// Base form props interface
interface BaseFormProps {
  serviceType: ServiceType;
  gitProviders?: any[];
  onSubmit: (data: MysqlServiceFormValues) => Promise<void>;
  loading: boolean;
  instanceTypeGroups?: InstanceTypeGroup[];
}

// MySQL service form component
export function MysqlServiceForm({ 
  serviceType,
  onSubmit,
  loading,
  instanceTypeGroups = [],
  ...
  const form = useForm<MysqlServiceFormValues>({
    resolver: zodResolver(mysqlServiceFormSchema),
    defaultValues: {
      projectId: projectId,
  ...
  useEffect(() => {
    if (storageCapacity) {
      ...
      if (isValid) {
        // First 10GB is free for MySQL and PostgreSQL services
