...
// Memory service form schema with storage capacity and instance type
const memoryServiceFormSchema = z.object({
...
// Form types
type MemoryServiceFormValues = z.infer<typeof memoryServiceFormSchema>;

// Base form props interface
interface BaseFormProps {
  serviceType: ServiceType;
  gitProviders?: any[];
  onSubmit: (data: MemoryServiceFormValues) => Promise<void>;
  loading: boolean;
  instanceTypeGroups?: InstanceTypeGroup[];
}

// Memory service form component
export function MemoryServiceForm({ 
  serviceType,
  onSubmit,
  loading,
  instanceTypeGroups = [],
  ...
  const form = useForm<MemoryServiceFormValues>({
    resolver: zodResolver(memoryServiceFormSchema),
    defaultValues: {
      projectId: projectId,
  ...
  const storageEnabled = form.watch('storageEnabled');
  const instanceTypeId = form.watch('instanceTypeId');
  
  useEffect(() => {
    if (storageCapacity && storageEnabled) {
      // Check if the storage value is valid (multiple of 5GB)
      const isValid = storageCapacity % 5 === 0;
      if (isValid) {
        setStoragePrice((storageCapacity * 0.05).toFixed(2)); // $0.05 per GB
      } else {
  ...
  }, [storageCapacity, storageEnabled]);
                ...
                <FormItem className="flex flex-row items-start space-x-3 space-y-0 p-4">
                  ...
                  <div className="space-y-1 leading-none">
                    ...
                    <p className="text-sm text-muted-foreground">
                      When enabled, Memory data will be persisted to disk
                    </p>
                  </div>
                  <FormMessage />
                </FormItem>
