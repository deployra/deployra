...
// Check if service needs privileged Kubernetes access (service account, service links)
function isPrivilegedService(serviceId: string | undefined): boolean {
	if (!serviceId) return false;
	return appConfig.privilegedServices.serviceIds.some(id => serviceId.includes(id));
}

export function createWebServiceDeploymentManifest(deploymentName: string, config: KubeDeploymentConfig, containerRegistrySecret: string | undefined, envSecretName?: string) {
	const isPrivileged = isPrivilegedService(config.serviceId);
	...
	const manifest = {
		...
		spec: {
			...
			template: {
				...
				spec: {
					enableServiceLinks: isPrivileged,
					automountServiceAccountToken: isPrivileged,
					containers: [{
...
export function createWebServiceDeploymentCompletePatch(config: KubeDeploymentConfig, secretName: string | undefined, envSecretName?: string): any {
	const isPrivileged = isPrivilegedService(config.serviceId);
...
export function createWebServiceServiceManifest(serviceName: string, config: KubeDeploymentConfig) {
	return {
		apiVersion: 'v1',
		kind: 'Service',
		metadata: {
			...
			labels: {
				...
				...(config.domains && config.domains.length > 0 
					? config.domains.reduce((acc, domain, index) => {
						acc[`domain-${index + 1}`] = domain;
						return acc;
...
export function createWebServiceHPAManifest(hpaName: string, config: KubeDeploymentConfig) {
	return {
...
export function createWebServicePersistentVolumeManifest(pvName: string, config: KubeDeploymentConfig) {
	return {
		apiVersion: 'v1',
...
export function createWebServicePersistentVolumeClaimManifest(pvcName: string, config: KubeDeploymentConfig) {
	return {
		apiVersion: 'v1',
