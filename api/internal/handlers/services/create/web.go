...
func createWebService(c *fiber.Ctx, user *models.User, req CreateServiceRequest, project models.Project, instanceType models.InstanceType) error {
	db := database.GetDatabase()

	// Generate subdomain
	subdomain := utils.GenerateSubdomain(req.Name)
	...
	if err := db.Create(&service).Error; err != nil {
		log.Printf("Error creating web service: %v", err)
		return response.InternalServerError(c, "Failed to create service")
	}

	// Create service ports if provided
	...
	log.Printf("Web service %s created", req.Name)
	...
	scaleToZeroEnabled := strings.Contains(service.InstanceTypeID, "free")
